...
{
    meta:
        description = "Detects PHP backdoor with hex-encoded command execution functions"
        author = "Grok (xAI)"
        date = "2025-03-13"
        threat_level = "High"

    strings:
        // Hex-encoded function names from the original code
        $func1 = "\\x73yste\\x6D"              // "system"
        $func2 = "\\x73hell\\x5Fexe\\x63"      // "shell_exec"
        $func3 = "\\x65\\x78ec"                // "exec"
        $func4 = "p\\x61\\x73\\x73\\x74hru"    // "passthru"
        $func5 = "p\\x6F\\x70e\\x6E"          // "popen"
        $func6 = "\\x73\\x74rea\\x6D_\\x67e\\x74_\\x63\\x6F\\x6Et\\x65nts" // "stream_get_contents"
        $func7 = "pcl\\x6F\\x73e"              // "pclose"

        // Specific variables and patterns
        $var1 = "$right_pad_string"            // Common variable prefix in the code
        $post_param = "\\x70\\x6Fi\\x6E\\x74er" // "pointer" hex-encoded in POST
        $hex_parser = "hex2bi\\x6E"            // "hex2bin" function

        // Unique function pattern for XOR decoding
        $xor_func = "function splitter_tool($hld,$token){  $element = '';    foreach(str_split($hld) as $char){$element.=chr(ord($char)^$token);} return$element;  }"

    condition:
        // Must have the XOR decoding function and at least 3 of the encoded functions
        $xor_func and 3 of ($func*)
        or
        // Or combination of POST parameter, hex parser, and at least 2 functions
        ($post_param and $hex_parser and 2 of ($func*))
        or
